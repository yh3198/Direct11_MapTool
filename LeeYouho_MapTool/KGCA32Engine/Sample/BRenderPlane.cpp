...
bool BRenderPlane::Create(ID3D11Device* pd3dDevice, const TCHAR* pLoadShaderFile)
{
	m_pPlaneShape = make_shared<BPlane>();
	m_pMiniMapRT = make_shared<BDxRT>();
	TCHAR* NullTex = NULL;
	m_pPlaneShape->Create(pd3dDevice, pLoadShaderFile, NullTex);
	m_pMiniMapRT->Create(pd3dDevice);
...
bool BRenderPlane::Render(ID3D11DeviceContext* pContext, ID3D11RenderTargetView* OldRenderTargetView, ID3D11DepthStencilView* OldDepthStencilView, ID3D11DepthStencilState* m_pDSS)
{
