...
VS_OUTPUT VS(VS_INPUT vIn)
{
	...
	float3 vNormal = normalize(mul(input.Normal, (float3x3)g_matNormal));
	output.vEye = normalize(cb_vEyeDir);

	float3 T = normalize(mul(vIn.Tangent, (float3x3)g_matNoraml);
	...
	float vDistance = distance(vWorldPos, g_vCameraPos.xyz); // 정점과 카메라 거리의 값
	output.FogDist.w = vIn.Position.y;
	output.FogDist.x = g_FogStart;
	output.FogDist.y = g_FogEnd;
	output.FogDist.z = vDistance;
...
float4 PS(VS_OUTPUT vIn) : SV_Target
{
	...
	float fValue = (-g_ClipPlane.w - vIn.FogDist.w) / (-g_ClipPlane.w - input.FogDist.w - 2.0f);
